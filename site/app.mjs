...
import {DEFAULTS,UNFILTERED,TASKS,LEVELS,MAX_COMPARE,evidenceCoverage,filterEntries,sortEntries,relativeDate,versionText,stateFromUrl,stateToUrl,comparisonFromUrl,comparisonToUrl,facetCounts,adaptTaskSelection,recoveryOptions} from './model.mjs?v=11';
...
function toggleCompare(entry,checked){lastCompareFocus=document.activeElement;if(checked){if(!comparison.includes(entry.id)&&comparison.length<MAX_COMPARE)comparison=[...comparison,entry.id];else if(comparison.length>=MAX_COMPARE){notify('Compare up to '+MAX_COMPARE+' resources at a time.');return;}}else comparison=comparison.filter(id=>id!==entry.id);syncUrl();render();}
function openComparison(){const entries=comparison.map(id=>data.entries.find(entry=>entry.id===id)).filter(Boolean);if(entries.length<2){notify('Select at least two resources to compare.');return;}const body=$('#compare-body');body.replaceChildren();const table=make('table',undefined,'compare-table');table.append(make('caption','Side-by-side comparison'));const head=make('thead'),headRow=make('tr');headRow.append(make('th','Resource'));for(const entry of entries)headRow.append(make('th',entry.name));head.append(headRow);table.append(head);const rows=[['Category',entry=>entry.category||'Not established'],['Format',entry=>entry.kind||'Not established'],['Access',entry=>entry.access||'Not established'],['Platforms',entry=>entry.platforms.join(' · ')||'Not established'],['Resolve edition',entry=>reqText(entry,'editions')],['Resolve version',entry=>reqText(entry,'resolve')],['Evidence',entry=>{const coverage=evidenceCoverage(entry);return coverage.established+' / '+coverage.total+' fields established';}]];const tbody=make('tbody');for(const [label,value] of rows){const row=make('tr');row.append(make('th',label));for(const entry of entries)row.append(make('td',value(entry)));tbody.append(row);}table.append(tbody);body.append(table);lastCompareFocus=document.activeElement;$('#compare-dialog').showModal();$('#compare-dialog-close').focus();}
function decorateCompareControls(found){for(const [index,article] of [...document.querySelectorAll('#results article.resource')].entries()){const entry=found[index];if(!entry)continue;const actions=article.querySelector('.row-actions');if(!actions||actions.querySelector('.compare-control'))continue;const control=make('label','Compare','compare-control'),input=make('input');input.type='checkbox';input.checked=comparison.includes(entry.id);input.disabled=!input.checked&&comparison.length>=MAX_COMPARE;input.setAttribute('aria-label','Compare '+entry.name);input.addEventListener('change',event=>toggleCompare(entry,event.target.checked));control.prepend(input);actions.append(control);}}
