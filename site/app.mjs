...
function setState(patch){
  ...
  state=next;limit=30;syncUrl();try{localStorage.setItem('open-resolve-setup',JSON.stringify({platform:state.platform,edition:state.edition,resolve:state.resolve}));}catch{}render();
...
async function details(summary){openDialog(summary.name);const request=++detailRequest;$('#dialog-body').append(make('p','Loading details…'));let e;try{e=await loadDetail(summary.id);}catch{if(request!==detailRequest||!$('#detail-dialog').open)return;const retry=make('button','Retry','outline');retry.addEventListener('click',()=>{const trigger=lastFocus;details(summary);lastFocus=trigger;});$('#dialog-body').replaceChildren(make('p','Details could not load. Your filters are unchanged.'),retry);return;}if(request!==detailRequest||!$('#detail-dialog').open)return;$('#dialog-body').replaceChildren();const b=$('#dialog-body'),coverage=evidenceCoverage(e);b.append(make('p',e.description),link('Open original resource ↗',e.url),facts(e),make('h3','Evidence coverage'),make('p','Established fields: '+coverage.established+' / '+coverage.total+'. This summary describes documented metadata, not blanket compatibility or installation testing.'),evidenceCoverageList(e));if(e.evidence.length)b.append(make('h3','Evidence sources and limitations'),evidenceList(e));if(e.history.length)b.append(make('h3','Version history'));if(e.history.length)for(const h of e.history){const p=make('p',h.from+' → '+h.to+' · '+h.summary+' ');p.append(link('Source',h.changelogs?.[0]?.url||h.source));b.append(p);b.append(changelogView(h));}const issue=new URL('https://github.com/subtlesayak/open-resolve-list/issues/new');issue.searchParams.set('template','creator-confirmation.yml');issue.searchParams.set('title','Confirm or correct: '+e.name);b.append(link('Confirm or correct this listing ↗',issue.href));}
function setupVerdict(e){const hasSetup=!!(state.platform||state.edition||state.resolve);if(!hasSetup)return null;const criteria={...UNFILTERED,official:state.official,platform:state.platform,edition:state.edition,resolve:state.resolve};if(!filterEntries([e],criteria).length)return {label:"Does not match your setup",kind:"no"};const unknown=[];if(state.platform&&!e.platforms.length)unknown.push("platform");if(state.edition&&!e.requirements.editions.length)unknown.push("edition");if(state.resolve&&!e.requirements.resolve.length)unknown.push("Resolve version");return unknown.length?{label:"Compatibility partly documented",kind:"maybe"}:{label:"Matches your setup",kind:"yes"};}
function basicRow(e){const article=make('article',undefined,'resource'),main=make('div'),h=make('h3');const titleLink=make('a',e.name);titleLink.href='resource/'+encodeURIComponent(e.id)+'/';h.append(titleLink);main.append(h,make('p',e.creator,'creator'),make('p',e.description,'description'));const tags=make('div',undefined,'resource-tags');if(e.kind)tags.append(make('span',FORMAT_LABELS[e.kind]||e.kind,'resource-tag'));for(const task of (e.tasks||[]).slice(0,2))if(TASKS[task])tags.append(make('span',TASKS[task],'resource-tag'));main.append(tags);const meta=make('div',undefined,'metadata');meta.append(make('p',e.reference?'📖 Reference':e.platforms.join(' · ')||'Platforms not established'),make('p',e.access),make('p',versionText(e.version),'version'));const coverage=evidenceCoverage(e),highest=e.evidence.some(x=>x.level==='tested')?'tested':e.evidence.some(x=>x.level==='creator')?'creator':e.evidence.length?'documented':'unknown';const ev=make('p',coverage.established+'/'+coverage.total+' evidence fields','evidence-label');ev.title='Field coverage; open Details for the individual fields and sources.';ev.setAttribute('aria-label',coverage.established+' of '+coverage.total+' evidence fields established; highest source level '+LEVELS[highest]);meta.append(ev);const setup=setupVerdict(e);if(setup)meta.append(make("p",setup.label,"setup-status setup-"+setup.kind));if(state.edition)meta.append(make('p',e.requirements.editions.length?(state.edition==='Studio'&&!e.requirements.editions.includes('Studio')?'Studio: inferred from Free compatibility; not separately verified':'Resolve edition: '+e.requirements.editions.join(' / ')):'Assumed compatible: edition not verified','edition-status'));if(state.sort==='stars')meta.append(make('p',e.stars===null?'Stars not applicable':'⭐ '+e.stars));if(state.sort==='updated')meta.append(make('p','Release: '+relativeDate(e.releaseDate)));if(state.sort==='activity')meta.append(make('p','Repo activity: '+relativeDate(e.activityDate)));const source=make('p',undefined,'source-link');source.append(link(e.official?'Official source ↗':'Creator site ↗',e.url));const actions=make('div',undefined,'row-actions');const button=make('button','Details','outline');button.setAttribute('aria-label','Details for '+e.name+' by '+e.creator);button.addEventListener('click',()=>details(e));actions.append(button);article.append(main,meta,source,actions);return article;}

const filterNames = {kind:'Format',platform:'Operating system',edition:'Resolve edition',resolve:'Resolve version',access:'Access',processing:'Processing',evidence:'Evidence',pricing:'Payment model',architecture:'Processor architecture',q:'Search',official:'Official BMD resources',task:'Task'};
...
function renderFilterSummary() {
  ...
  for (const el of active) {
    ...
    const value = el.type === 'checkbox' ? (el.name==='official'?'Show':'Include') : originalOptions.get(el.name)?.find(o=>o.value===state[el.name])?.label || state[el.name];
...
async function init(){const controls=[...document.querySelectorAll('#search, .modes button, #filters select, #filters input, #clear, #share, #sort')];controls.forEach(el=>el.disabled=true);try{const r=await fetch('catalogue-index.json?v=1',{cache:'no-store'});if(!r.ok)throw Error('HTTP '+r.status);data=await r.json();for(const e of data.entries)e.evidence=e.evidenceLevels.map(level=>({level}));for(const [value,label]of Object.entries(FORMAT_LABELS))if(data.entries.some(e=>e.kind===value))$('#filters [name=kind]').append(new Option(label,value));for(const value of collectResolveVersions(data.entries))$('#filters [name=resolve]').append(new Option(value,value));$('.brand').textContent=data.title;document.title=data.title+' — Find tools for your setup';for(const [v,t]of Object.entries(TASKS)){$('#filters [name=task]').append(new Option(t,v));}const restored=stateFromUrl(location.search);state=location.search?restored.state:UNFILTERED;try{if(!location.search){const saved=JSON.parse(localStorage.getItem("open-resolve-setup")||"null");if(saved&&typeof saved==="object")state={...state,platform:["Windows","macOS","Linux","iPadOS"].includes(saved.platform)?saved.platform:"",edition:["","Free","Studio"].includes(saved.edition)?saved.edition:"",resolve:typeof saved.resolve==="string"?saved.resolve:""};}}catch{}for(const el of [...$('#filters').elements,$('#sort')])if(el.tagName==='SELECT'){const key=el.name||'sort';if(![...el.options].some(o=>o.value===state[key]))state[key]=DEFAULTS[key];}if(!['all','tested'].includes(state.mode))state.mode='all';$('#filter-toggle').addEventListener('click',()=>{const closed=$('#filter-controls').dataset.collapsed==='true';$('#filter-controls').dataset.collapsed=String(!closed);$('#filter-toggle').setAttribute('aria-expanded',String(closed));});$('#search').addEventListener('input',e=>setState({q:e.target.value}));$('#filters').addEventListener('submit',e=>e.preventDefault());$('#filters').addEventListener('change',e=>{if(e.target.name)setState({[e.target.name]:e.target.type==='checkbox'?(e.target.name==='official'?(e.target.checked?'show':'hide'):(e.target.checked?e.target.value:'')):e.target.value});});$('#sort').addEventListener('change',e=>setState({sort:e.target.value}));$('#clear').addEventListener('click',()=>{setState({...DEFAULTS,q:state.q,sort:state.sort});notify('Filters cleared.');});$('#all-mode').addEventListener('click',()=>setState({mode:'all'}));$('#tested-mode').addEventListener('click',()=>setState({mode:'tested'}));$('#more').addEventListener('click',()=>{limit+=30;render();});$('#share').addEventListener('click',async()=>{try{await navigator.clipboard.writeText(location.href);notify('View link copied.');}catch{notify('Copy the address from your browser to share this view.');}});enableBackdropClose($('#detail-dialog'));$('#dialog-close').addEventListener('click',()=>$('#detail-dialog').close());$('#detail-dialog').addEventListener('close',()=>lastFocus?.focus());window.addEventListener('popstate',()=>{const restored=stateFromUrl(location.search);state=location.search?restored.state:UNFILTERED;try{if(!location.search){const saved=JSON.parse(localStorage.getItem("open-resolve-setup")||"null");if(saved&&typeof saved==="object")state={...state,platform:["Windows","macOS","Linux","iPadOS"].includes(saved.platform)?saved.platform:"",edition:["","Free","Studio"].includes(saved.edition)?saved.edition:"",resolve:typeof saved.resolve==="string"?saved.resolve:""};}}catch{}limit=30;render();});controls.forEach(el=>el.disabled=false);render();}catch(error){$('#count').textContent='The catalogue could not load.';$('#results').append(make('p','Reload the page, or use the complete GitHub catalogue. This site needs an HTTP server rather than opening index.html directly.'),link('Open catalogue ↗','https://github.com/subtlesayak/open-resolve-list'));console.error(error);}}
...
function row(e){const article=basicRow(e);const creator=article.querySelector('.creator');if(creator&&e.creator){const anchor=document.createElement('a');anchor.href='creator/'+e.creator.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')+'/';anchor.textContent=e.creator;creator.replaceChildren(anchor);}const evidence=article.querySelector('.evidence-label');if(evidence){const coverage=evidence.getAttribute('aria-label')||'';const match=coverage.match(/(\d+) of (\d+)/);if(match){const established=Number(match[1]),total=Number(match[2]),confidence=established>=6?'High':established>=3?'Partial':'Limited';evidence.textContent='Compatibility confidence: '+confidence;evidence.title=established+' of '+total+' compatibility details documented. Unknown does not mean incompatible.';evidence.setAttribute('aria-label','Compatibility confidence '+confidence+'; '+established+' of '+total+' details documented');}}return article;}
