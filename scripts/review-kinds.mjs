...
const kind=(process.argv.includes('--kind')?process.argv[process.argv.indexOf('--kind')+1]:null)||'other';
...
if(process.argv.includes('--write-report')){
  ...
  const report={schema_version:1,review_date:new Date().toISOString().slice(0,10),approved_mappings:Object.keys(APPROVED_KIND_REVIEW).length,retained_other:records.map(record=>({...record,decision:'retain-other',reason:'Maintained source evidence does not establish a specific supported package format.'}))};
