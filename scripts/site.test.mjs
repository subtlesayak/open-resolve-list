...
test('hiding official BMD listings preserves third-party resources and shared preferences',()=>{
  ...
  assert.equal(filterEntries(data.entries,{...DEFAULTS}).length,484);
...
test('static resource directories expose canonical SEO pages without private data',()=>{const sample=data.entries[0],root=new URL('../site/resource/',import.meta.url),file=new URL(`${sample.id}/index.html`,root),html=fs.readFileSync(file,'utf8');assert.equal(fs.readdirSync(root).length,514);assert.match(html,new RegExp(`<h1>${sample.name.replace(/[.*+?^${}()|[\\]\\\\]/g,'\\\\$&')}</h1>`));assert.match(html,/application\/ld\+json/);assert.match(html,/rel="canonical"/);assert.match(html,/Sources and limitations/);assert.doesNotMatch(html,/C:\\\\Users|D:\\\\Projects|gmail_draft_id|contact_email/);for(const entry of data.entries){const page=fs.readFileSync(new URL(`${entry.id}/index.html`,root),'utf8');assert.match(page,/<h1>[^<]+<\/h1>/,entry.id);assert.match(page,/rel="canonical"/,entry.id);assert.doesNotMatch(page,/C:\\\\Users|D:\\\\Projects|gmail_draft_id|contact_email/,entry.id);}});
