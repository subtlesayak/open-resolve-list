...
test('HTML directory exposes every resource through a crawlable link',()=>{
 const directory=fs.readFileSync(new URL('../site/directory.html',import.meta.url),'utf8');
 const home=fs.readFileSync(new URL('../site/index.html',import.meta.url),'utf8');
 assert.match(home,/href="directory.html"/);
 for(const entry of data.entries)assert.ok(directory.includes(`href="resource/${encodeURIComponent(entry.id)}/"`),entry.id);
 assert.match(directory,/<link rel="canonical" href="https:\/\/subtlesayak.github.io\/open-resolve-list\/directory.html"/);
});
test('public release API preserves original GitHub release links',()=>{
 ...
 for(const release of api.releases)assert.match(release.source,/^https:\/\/github\.com\/subtlesayak\/open-resolve-list\/releases\/tag\//);
});
...
test('hiding official BMD listings preserves third-party resources and shared preferences',()=>{
  ...
  assert.equal(filterEntries(data.entries,{...DEFAULTS}).length,485);
  assert.equal(DEFAULTS.sort,'name');
});
test('website contains all 515 unique catalogue entries and no private outreach data',()=>{assert.equal(data.entries.length,515);assert.equal(new Set(data.entries.map(e=>e.id)).size,515);assert.equal(new Set(data.entries.map(e=>e.url)).size,515);const json=JSON.stringify(data);assert.doesNotMatch(json,/C:\\\\Users|D:\\\\Projects|gmail_draft_id|contact_email|gmail-receipts/);});
...
test('individual resource page uses the per-resource API without publishing private data',()=>{const html=fs.readFileSync(new URL('../site/resource.html',import.meta.url),'utf8');const script=fs.readFileSync(new URL('../site/resource.mjs',import.meta.url),'utf8');assert.match(html,/resource\.mjs/);assert.match(script,/createDetailLoader/);assert.match(script,/canonical/);assert.match(script,/resourceView/);assert.doesNotMatch(script,/C:\\\\Users|D:\\\\Projects|gmail_draft_id|contact_email/);});
test('static resource directories expose canonical SEO pages without private data',()=>{const sample=data.entries[0],root=new URL('../site/resource/',import.meta.url),file=new URL(`${sample.id}/index.html`,root),html=fs.readFileSync(file,'utf8');assert.equal(fs.readdirSync(root).length,515);assert.match(html,new RegExp(`<h1>${sample.name.replace(/[.*+?^${}()|[\\]\\\\]/g,'\\\\$&')}</h1>`));assert.match(html,/application\/ld\+json/);assert.match(html,/rel="canonical"/);assert.match(html,/Sources and limitations/);assert.doesNotMatch(html,/C:\\\\Users|D:\\\\Projects|gmail_draft_id|contact_email/);for(const entry of data.entries){const page=fs.readFileSync(new URL(`${entry.id}/index.html`,root),'utf8');assert.match(page,/<h1>[^<]+<\/h1>/,entry.id);assert.match(page,/rel="canonical"/,entry.id);assert.doesNotMatch(page,/C:\\\\Users|D:\\\\Projects|gmail_draft_id|contact_email/,entry.id);}});
test('machine-readable resource feed is deterministic and canonical-ID based',()=>{const feed=JSON.parse(fs.readFileSync(new URL('../site/resources.json',import.meta.url)));const indexHtml=fs.readFileSync(new URL('../site/index.html',import.meta.url),'utf8');const resourceHtml=fs.readFileSync(new URL('../site/resource.html',import.meta.url),'utf8');assert.equal(feed.schema_version,1);assert.equal(feed.resources.length,515);assert.equal(new Set(feed.resources.map(entry=>entry.id)).size,515);assert.ok(feed.resources.every(entry=>entry.kind&&entry.url.startsWith('https://')&&entry.description));assert.match(indexHtml,/rel="alternate" type="application\/json" href="resources\.json"/);assert.match(resourceHtml,/rel="alternate" type="application\/json" href="resources\.json"/);assert.doesNotMatch(JSON.stringify(feed),/C:\\\\Users|D:\\\\Projects|gmail_draft_id|contact_email/);});
test('static API, RSS and sitemap outputs are deterministic and source-backed',()=>{const api=JSON.parse(fs.readFileSync(new URL('../site/api/v1/resources.json',import.meta.url)));const detail=JSON.parse(fs.readFileSync(new URL('../site/api/v1/resources/36-cinematic-film-titles.json',import.meta.url)));const categories=JSON.parse(fs.readFileSync(new URL('../site/api/v1/categories.json',import.meta.url)));const tasks=JSON.parse(fs.readFileSync(new URL('../site/api/v1/tasks.json',import.meta.url)));const releases=JSON.parse(fs.readFileSync(new URL('../site/api/v1/releases.json',import.meta.url)));const rss=fs.readFileSync(new URL('../site/feed.xml',import.meta.url),'utf8');const sitemap=fs.readFileSync(new URL('../site/sitemap.xml',import.meta.url),'utf8');const robots=fs.readFileSync(new URL('../site/robots.txt',import.meta.url),'utf8');assert.equal(api.resources.length,515);assert.equal(detail.resource.id,'36-cinematic-film-titles');assert.ok(detail.resource.history);assert.ok(categories.categories.length>0);assert.equal(tasks.tasks.length,Object.keys(INITIAL_DEFAULTS).includes('q')?10:10);assert.ok(releases.releases.length>0);assert.match(rss,/^<\?xml version="1\.0" encoding="UTF-8"\?>/);assert.match(rss,/<rss version="2\.0">/);const creatorPages=fs.readdirSync(new URL('../site/creator/',import.meta.url)).length;assert.equal((sitemap.match(/<loc>/g)||[]).length,api.resources.length+5+creatorPages);assert.ok(sitemap.includes("about.html"));assert.ok(sitemap.includes("updates.html"));assert.match(sitemap,/resource\/36-cinematic-film-titles\//);assert.match(robots,/Sitemap: https:\/\/subtlesayak\.github\.io\/open-resolve-list\/sitemap\.xml/);assert.doesNotMatch(JSON.stringify({api,detail,categories,tasks,releases,rss,sitemap,robots}),/C:\\\\Users|D:\\\\Projects|gmail_draft_id|contact_email/);});
test('local rebuild entry point is explicit and publication-free',()=>{const script=fs.readFileSync(new URL('./rebuild.mjs',import.meta.url),'utf8');assert.match(script,/validate-resources\.mjs/);assert.match(script,/build-catalogue\.mjs/);assert.match(script,/build-site\.mjs/);assert.match(script,/No publishing or upload/);assert.doesNotMatch(script,/git push|gh release|workflow_dispatch/);});
...
test('search finds editorial concepts and spelling variants without broad category false positives',()=>{
 ...
 assert.ok(results('film').includes('utility-dctls')); // Category/task terms are searchable; exact tags still rank higher.
