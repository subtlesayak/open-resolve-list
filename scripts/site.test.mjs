...
import {DEFAULTS as INITIAL_DEFAULTS,UNFILTERED as DEFAULTS,EVIDENCE_FIELDS,evidenceCoverage,searchScore,filterEntries,sortEntries,stateFromUrl,stateToUrl,comparisonFromUrl,comparisonToUrl,comparableResources,comparisonCompatible,matchesVersion,relativeDate,facetCounts,adaptTaskSelection,recoveryOptions,normalizeSearch} from '../site/model.mjs';
...
test('comparison only accepts similar formats or shared category and task',()=>{const makeEntry=(id,kind,category,tasks)=>({id,kind,category,tasks});const dctl=makeEntry('a','dctl','Color tools',['color']);const dctl2=makeEntry('b','dctl','Other',['fusion']);const lut=makeEntry('c','lut','Color tools',['color']);const audio=makeEntry('d','vst','Audio tools',['audio']);assert.equal(comparableResources(dctl,dctl2),true);assert.equal(comparableResources(dctl,lut),true);assert.equal(comparableResources(dctl,audio),false);assert.equal(comparisonCompatible(lut,[dctl]),true);assert.equal(comparisonCompatible(audio,[dctl]),false);});
...
test('website generation is deterministic and exposes similar-resource comparison controls',()=>{const url=new URL('../site/catalogue.json',import.meta.url),before=fs.readFileSync(url,'utf8');buildSite();assert.equal(fs.readFileSync(url,'utf8'),before);const html=fs.readFileSync(new URL('../site/index.html',import.meta.url),'utf8');const script=fs.readFileSync(new URL('../site/app.mjs',import.meta.url),'utf8');assert.match(html,/compare-tray/);assert.match(html,/Compare selected/);assert.match(script,/similar resources/);assert.match(script,/comparisonCompatible/);assert.match(html,/name="processing"/);assert.equal(relativeDate('2026-08-07',new Date('2026-09-07')),'1 month back');});
