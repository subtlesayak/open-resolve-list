...
const allowedKinds=new Set(['dctl','ofx','powergrade','fuse','fusion-macro','reactor-package','lut','resolve-script','mcp-server','workflow-app','subtitle-tool','template','encoder','control-surface','reference','collection','other']);
...
async function main(){
 ...
 const draft={id:null,name:String(values.name),creator:String(values.creator),urls:{canonical:url.href,previous:[]},origin:url.hostname==='github.com'?'github':'external',category:String(values.category),kind:String(values.kind),tasks,description:String(values.description),access:String(values.access||'public'),platforms:[],platform_notes:'',requirements:{editions:[],resolve:[],architectures:[],gpu:null,processing:'unknown',pricing:'unknown',account:'unknown',dependencies:null,installation:null},version:{kind:'unknown',version:null,date:null,date_kind:'unknown',source:url.href,checked_at:null},evidence:[],history:[],review_markers:['Draft requires upstream evidence and canonical migration before inclusion.']};
